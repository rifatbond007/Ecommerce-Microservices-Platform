...
/**
 * Middleware that gates every `/api/v1/notifications/*` request on a
 * valid inter-service HMAC, so an attacker who reaches
 * notification-service's port directly cannot forge `x-user-id` /
 * `x-user-email` headers.
 *
 * No allow-list — every notification endpoint is gated. Mounted
 * BEFORE the per-route auth middleware so unsigned requests are
 * rejected before any controller code runs.
 *
