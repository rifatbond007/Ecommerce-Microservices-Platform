...
export async function consumeEvents(
  ...
  try {
    const notificationQueue = await channel.assertQueue('notification-service-queue', { durable: true });

    for (const binding of bindings) {
      await channel.bindQueue(notificationQueue.queue, EXCHANGE_NAME, binding.routingKey);
    }

    await channel.consume(notificationQueue.queue, async (msg) => {
      if (!msg) return;
