...
/**
 * Middleware that gates every `/api/v1/orders/*` request on a valid
 * inter-service HMAC, so an attacker who reaches order-service's port
 * directly cannot forge `x-user-id` / `x-user-email` headers.
 *
 * No allow-list — every order endpoint is gated. Mounted BEFORE the
