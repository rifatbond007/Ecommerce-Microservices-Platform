...
export const authenticate = async (
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      // The auth service's verifyInterService middleware guards every
      // /api/v1/auth/* route — including /me. We MUST send the same
      // HMAC headers the gateway would, otherwise the auth service
      // returns 401 INTER_SERVICE_SIGNATURE_INVALID and we silently
      // downgrade every authenticated request to "Invalid or expired
      // token".
