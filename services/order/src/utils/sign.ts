...
/**
 * Inter-service HMAC signing. Used by order-service when it calls
 * other services directly (e.g. order → cart to fetch cart contents
 * before creating an order). Algorithm MUST match
 * `services/gateway/src/utils/sign.ts` and every downstream `verify.ts`.
 */
