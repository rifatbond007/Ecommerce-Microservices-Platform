...
/**
 * Inter-service signature verification. See services/auth/src/utils/verify.ts
 * for the full rationale. Algorithm MUST match the gateway's sign.ts.
 *
 * IMPORTANT: the payment service allow-lists `/api/v1/webhooks/*` in
 * its inter-service middleware because webhooks come from Stripe with
 * their own `stripe-signature` header — they're not gateway-routed.
 */
