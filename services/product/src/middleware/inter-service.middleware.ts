...
/**
 * Middleware that gates every `/api/v1/products/*`,
 * `/api/v1/categories/*`, `/api/v1/brands/*`, `/api/v1/variants/*`,
 * and `/api/v1/inventory/*` request on a valid inter-service HMAC, so
 * an attacker who reaches product-service's port directly cannot
 * forge `x-user-id` / `x-user-email` headers.
 *
 * No allow-list — every product endpoint is gated. Mounted BEFORE the
