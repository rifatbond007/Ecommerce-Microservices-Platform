...
/**
 * Inter-service HMAC signing. The admin service already routes its
 * inter-service calls through the gateway (PR #9), which auto-signs
 * outbound — so we don't strictly need this helper. Kept here for
 * parity with the other services and for any future direct call.
 *
 * Algorithm MUST match `services/gateway/src/utils/sign.ts` and every
 * downstream `verify.ts`.
 */
