...
describe('inter-service verify helper (admin)', () => {
  ...
  it('accepts a valid signature for a POST with a JSON body', () => {
    ...
    const { signature, timestamp, keyId } = signRequest({
      method: 'POST',
      path: '/api/v1/admin/orders/abc/status',
      body,
      timestamp: NOW,
      keyId: 'v1',
    });
...
describe('auth middleware outbound /me signing (admin)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockedAxios.get.mockResolvedValue({
      data: { data: { id: 'u1', email: 'u@example.com', role: 'admin', sellerStatus: 'NONE' } },
    } as never);
  });

  it('sends x-inter-service-* headers on /me call when both headers and Bearer are present', async () => {
    const req = {
      headers: {
