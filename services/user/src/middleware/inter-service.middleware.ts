...
/**
 * Middleware that gates every `/api/v1/users/*` and `/api/v1/sellers/*`
 * request on a valid inter-service HMAC, so an attacker who reaches
 * user-service's port directly cannot forge `x-user-id` / `x-user-email`
 * headers.
 *
 * No allow-list — every user endpoint is gated. Mounted BEFORE the
